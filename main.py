...
def app():
    ...
    if video_file is not None:
        ...
        def start_processing():
            ...
            h5_path = os.path.join(tmp_dir, video_name + ".h5")
            cmd = f"python prepare_data_2d_custom.py -i ../inference/output_directory -o myvideos"
            output = subprocess.getoutput(cmd)
            put_code(output)
            ...
            # 显示一个下载按钮，让用户可以下载可视化的结果
            put_file(gif_path, os.path.getsize(gif_path))

            # 定义一个函数来计算投影损失并进行迭代优化
            def optimize():
               # 创建一个进度条，用来显示优化进度
               put_processbar('optimizing')
               # 加载原始的3D关节数据，假设它的形状是(N, J, 3)
               original_3d = np.load('output.npy')
               ...
               # 加载图像中的2D关键点数据，假设它的形状是(N, J, 2)，其中N是帧数，J是关节数，2是坐标维度
               image_2d = np.load('output_2d.npy')
               ...
               # 显示优化后的损失值和相机外参矩阵
               put_text(f"优化后的损失值：{optimized_loss:.4f}, 优化后的相机外参矩阵：{optimized_T}")
